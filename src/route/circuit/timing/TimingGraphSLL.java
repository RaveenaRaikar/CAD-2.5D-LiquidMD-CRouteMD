...
import route.circuit.timing.TimingGraph.SCC;
...
public class TimingGraphSLL {
    ...
    private List<GlobalBlock> globalBlockList = new ArrayList<>();
    private HashMap<BlockType,List<AbstractBlock>> allBlocks = new HashMap<>();
//    private List<AbstractBlock> allBlocks = new ArrayList<>();
    
    private TimingNode criticalNode;
    ...
    private List<AbstractPin> allSLLPins = new ArrayList<>();
    ...
    public TimingGraphSLL(Circuit[] circuit, HashMap<String, SllNetData> sllInfo, int totDie) {
        ...
        for(BlockType leafBlockType : BlockType.getLeafBlockTypes()) {
        	this.allBlocks.put(leafBlockType, new ArrayList<>());
//        	this.allBlocks.addAll(this.circuitdie[i].getBlocks(leafBlockType));
        }

        for(int i = 0; i < this.totaldie; i++) {
        	this.globalBlockList.addAll(this.circuitdie[i].getGlobalBlocks());
            for(BlockType leafBlockType : BlockType.getLeafBlockTypes()) {
            	if(this.allBlocks.containsKey(leafBlockType)) {
            		this.allBlocks.get(leafBlockType).addAll(this.circuitdie[i].getBlocks(leafBlockType));
            	}else {
            		this.allBlocks.put(leafBlockType, this.circuitdie[i].getBlocks(leafBlockType));
            	}
...
//        this.buildGraph();
        ...
        System.out.println("  Total Num clock domains " + this.numClockDomains);
    ...
    public void addAbstractPins(List<AbstractPin> allLeafPins) {
    	this.allSLLPins.addAll(allLeafPins);
    }
    ...
    private void buildSystemGraph() {
        ...
        for(BlockType leafBlockType : BlockType.getLeafBlockTypes()) {
            ...
            for(AbstractBlock abstractBlock : this.allBlocks.get(leafBlockType)) {
        ...
        for(GlobalBlock globalBlock : this.globalBlockList) {
    ...
    public void addSLLConnections(List<Connection> sllConnections) {
    	...
    	this.allConnections.addAll(sllConnections);
    }
    ...
    private void traverseFromSource(TimingNode pathSourceNode, float clockDelay) {
        ...
        while(!todo.empty()) {
            ...
            if((sourcePin.hasTimingNode() && sourcePin != pathSourcePin) || this.isEndpin(sourceBlock, sourcePin, pathSourcePin)) {
                ...
                if(sourceBlock.isGlobal() && pathSourcePin.getOwner().isGlobal()) {
                	GlobalPin sinkGlobalPin = (GlobalPin) sourcePin;
                	GlobalPin sourceGlobalPin = (GlobalPin) pathSourcePin;
                	if(sinkGlobalPin.getGlobalNetPin() && sourceGlobalPin.getGlobalNetPin()) {
                		disableEdge = true;
                	}
                }
                ...
                if(pathSinkNode != null && !disableEdge) {
                	GlobalBlock pathSinkBlock = pathSinkNode.getGlobalBlock();
                    TimingEdge edge = pathSourceNode.addSink(pathSinkNode, delay, this.circuitdie[0].getArchitecture().getDelayTables());
    ...
    public String printMaxPath() {
    	...
    	for(TimingEdge edge: this.timingEdges) {
    		if(edge.getSLLEdgeStatus()) {
    			TimingNode currentNode = edge.getSink();
    			criticalPath.add(currentNode);
    			while(currentNode.getArrivalTime() !=0) {
    				for(TimingEdge Innedge: currentNode.getSourceEdges()){
    					if(this.compareFloat(edge.getSource().getArrivalTime(), currentNode.getArrivalTime() - edge.getTotalDelay())){
    ...
    public void printTimingEdges() {
    	for(TimingEdge edge : this.timingEdges) {
    		...
    		if(edge.getSLLEdgeStatus()) {
        		...
        		System.out.print("\nThe sink is " + sinkNodePin.getPin() + " and the arrival time is " + ((sinkNodePin.getArrivalTime())*Math.pow(10, 9)));
    		}
    ...
    private void setRootAndLeafNodes(){
    	...
    	for(int clockDomain = 0; clockDomain < this.numClockDomains; clockDomain++) {
	        ...
	        System.out.print("\nClock Domain " + clockDomain + " has " + clockDomainLeafNodes.size()+ " leaf nodes");
    ...
    public void calculateRegionalWireDelay(List<Connection> conn) {
    ...
    public float calculateArrivalRequiredCritPerRegion(float maxCriticality, float criticalityExponent, List<Connection> conn, int region) {
        
        float maxDelay = 0;

        // Synchronize the entire connection list processing to prevent race conditions
        synchronized(conn) {
            for (Connection connection : conn) {
        ...
        for (int sourceClockDomain = 0; sourceClockDomain < this.numClockDomains; sourceClockDomain++) {
            for (int sinkClockDomain = 0; sinkClockDomain < this.numClockDomains; sinkClockDomain++) {
                if (this.includeClockDomain(sourceClockDomain, sinkClockDomain)) {
                    ...
                    // Synchronize the maxDelay update
                    synchronized (this) {
                        maxDelay = 0;
                    }
                    ...
                    synchronized (this) {
                        ...
                        for (TimingNode leafNode : clockDomainLeafNodes) {
                            ...
                            // Synchronize the maxDelay update to ensure atomicity
                            synchronized (this) {
		...
		for(TimingNode node : this.timingNodes) {		
    		if(node.getPosition() == Position.LEAF) {
    			node.setSourceClockDomains();
    				...
    				node.setSinkClockDomainsSystem();
    		}
    	}
	...
	public void printDelayAllNodes() {
		for(int i = 0 ; i < this.numClockDomains ; i++) {
			for(TimingNode node: this.leafNodes.get(i)) {
				System.out.print("\nThe max delay is " + (node.getArrivalTime() - node.clockDelay) + " for node " + node.getPin() +" for clock domain " + i);
			}
		}
    ...
    public String criticalPathToString() {
    	...
    	List<TimingNode> criticalPath = new ArrayList<>();
		TimingNode node = this.getEndNodeOfCriticalPath(sourceClockDomain, sinkClockDomain);
    	...
    	int maxLen = 25;
    	for(TimingNode criticalNode:criticalPath){
    		//System.out.print("\nThe node is " + criticalNode.toString().length());
    		if(criticalNode.toString().length() > maxLen){
    ...
    private TimingNode getEndNodeOfCriticalPath(int sourceClockDomain, int sinkClockDomain){
    	float delay;
    	for(TimingNode leafNode: this.leafNodes.get(sinkClockDomain)){
    		leafNode.resetArrivalTime();
    		leafNode.recursiveArrivalTime(sourceClockDomain);
    		if(compareFloat((leafNode.getArrivalTime() - leafNode.clockDelay), this.globalMaxDelay)){
    			return leafNode;
    		}
    	}
    	return null;
